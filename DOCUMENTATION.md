...
This document provides a comprehensive technical overview of the mathematics, algorithms, and architecture implemented in **Pyton3D**.

---

## Table of Contents
1. [Core Mathematics](#1-core-mathematics)
2. [Rigid Body Kinematics & 6-DOF Dynamics](#2-rigid-body-kinematics--6-dof-dynamics)
3. [Collision Detection Pipeline](#3-collision-detection-pipeline)
4. [Impulse Resolution & Friction Solver](#4-impulse-resolution--friction-solver)
5. [Numerical Integration Methods](#5-numerical-integration-methods)
6. [Constraints & Force Generators](#6-constraints--force-generators)
...
### 1.1 Vector3 (`Vec3`)
Represents points, velocities, accelerations, and forces in 3D Euclidean space:
$$\vec{v} = \begin{bmatrix} x \\ y \\ z \end{bmatrix}$$
Standard vector operations include dot product $\vec{a} \cdot \vec{b}$, cross product $\vec{a} \times \vec{b}$, Euclidean norm $\|\vec{v}\| = \sqrt{x^2 + y^2 + z^2}$, and normalized unit vectors $\hat{v} = \frac{\vec{v}}{\|\vec{v}\|}$.

### 1.2 Quaternions (`Quaternion`) & Rodrigues Rotation
Rotations are parameterized by unit quaternions:
$$\mathbf{q} = (x, y, z, w) = (\vec{q}_v, q_w)$$
where $\|\mathbf{q}\| = 1$.

#### Rodrigues Vector Rotation Formula
To rotate a vector $\vec{v}$ by quaternion $\mathbf{q}$ without constructing a full rotation matrix:
$$\vec{v}\' = \vec{v} + 2 q_w (\vec{q}_v \times \vec{v}) + 2 (\vec{q}_v \times (\vec{q}_v \times \vec{v}))$$
This inlined formula executes in **15 floating-point operations**, compared to 32 FLOPs for the full quaternion sandwich multiplication $\mathbf{q} \mathbf{v} \mathbf{q}^*$.

### 1.3 3x3 Inertia Matrix (`Mat3`)
Rigid body mass distributions are represented by $3 \times 3$ symmetric inertia tensors $\mathbf{I}$. For a box with half-extents $(h_x, h_y, h_z)$ and mass $m$:
...
### 3.1 Broad-Phase
Axis-Aligned Bounding Box (AABB) intersection tests reject non-overlapping pairs in $O(1)$ per pair:
$$\text{Overlap} = (\min_A \le \max_B) \land (\max_A \ge \min_B) \quad \forall \, x,y,z$$

### 3.2 Narrow-Phase: Separating Axis Theorem (SAT)
For two Oriented Bounding Boxes (OBBs) $A$ and $B$, there are **15 potential separating axes**:
...
For each candidate unit axis $\hat{L}$, project the centers and extents:
...
If $d > r_A + r_B$, the boxes are separated. Otherwise, the penetration depth is $p = (r_A + r_B) - d$. The axis corresponding to the minimum penetration depth is chosen as the contact normal.

---

## 4. Impulse Resolution & Friction Solver

### 4.1 Relative Velocity
At contact point $\vec{p}$:
...
### 4.4 2-Axis Coulomb Friction
Two orthogonal tangent vectors $\hat{t}_1, \hat{t}_2$ are constructed perpendicular to $\hat{n}$. The frictional impulse along tangent $k$ is clamped to the Coulomb friction cone:
$$|j_{t,k}| \le \mu j_n$$

### 4.5 Baumgarte Positional Recovery
To eliminate penetration drift without adding artificial kinetic energy:
...
## 6. Constraints & Force Generators
...
Pyton3D scenes are serialized as JSON:
